...
CMD ["sh", "-c", "if [ \"$SERVICE\" = \"api\" ]; then python -m app.check_db --max-attempts 10 --wait-time 10 && python -c 'from app.models import create_tables; create_tables()' && uvicorn app.main:app --host 0.0.0.0 --port $PORT; elif [ \"$SERVICE\" = \"worker\" ]; then python -m app.check_db --max-attempts 10 --wait-time 10 && celery -A app.tasks worker --loglevel=info; elif [ \"$SERVICE\" = \"beat\" ]; then python -m app.check_db --max-attempts 10 --wait-time 10 && celery -A app.tasks beat --loglevel=info; elif [ \"$SERVICE\" = \"flower\" ]; then python -m app.check_db --max-attempts 10 --wait-time 10 --skip-db && celery -A app.tasks flower --port=$PORT; fi"]